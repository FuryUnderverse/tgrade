...
func TestGlobalFee(t *testing.T) {
	...
	txResult = cli.CustomCommand("tx", "wasm", "store", anyContract, "--from=node0", "--fees=101furya")
	RequireTxFailure(t, txResult)

	t.Log("Transactions with too high fees should fail (gas)")
	txResult = cli.CustomCommand("tx", "wasm", "store", anyContract, "--from=node0", "--gas=101", "--gas-prices=1furya")
	RequireTxFailure(t, txResult)
}
