...
func TestProofOfEngagementSetup(t *testing.T) {
	...
	sut.withEachNodeHome(func(i int, home string) {
		clix := NewPetriCliX(t, sut.rpcAddr, sut.chainID, filepath.Join(workDir, home), verbose)
	...
	eResult := cli.Execute(engagementGroupAddr, engagementUpdateMsg.ToJSON(t), pt4BootstrapAccountAddr)
