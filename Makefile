...
build-docker:
	$(DOCKER) build -t oldfurya/furya:local .
...
format: format-tools
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs goimports -w -local github.com/oldfurya/furya
